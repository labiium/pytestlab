...
class AWGAccuracy(BaseModel):
    ...
    amplitude: float = Field(
        ..., description="Amplitude accuracy specification (e.g., percentage or absolute value)"
    )
...
class ArbitraryWaveformConfig(BaseModel):
    model_config = ConfigDict(validate_assignment=True, extra="forbid")
    memory: float = Field(
        ..., description="Arbitrary waveform memory size (e.g., in points or bytes)"
    )
    max_length: float = Field(
        ..., description="Maximum length of a single arbitrary waveform segment (in points)"
    )
    sampling_rate: Range = Field(
        ..., description="Programmable sampling rate range for arbitrary waveforms"
    )
    resolution: int = Field(
        ..., gt=0, description="Vertical resolution for arbitrary waveforms in bits"
    )
...
class WaveformsConfig(BaseModel):
    ...
    built_in: list[str] = Field(
        ...,
        min_length=1,
        description="List of available built-in waveform shapes (e.g., SINE, SQUARE)",
    )
    arbitrary: ArbitraryWaveformConfig = Field(
        ..., description="Configuration for arbitrary waveform capabilities"
    )
...
class WaveformGeneratorConfig(InstrumentConfig):
    ...
    device_type: Literal["AWG", "waveform_generator"] = Field(
        "waveform_generator", description="Type of the device (e.g., 'AWG', 'waveform_generator')"
    )
    channels: list[AWGChannelConfig] = Field(
        ..., min_length=1, description="List of waveform generator channel configurations"
    )
