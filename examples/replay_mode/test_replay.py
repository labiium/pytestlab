...
async def test_replay_backend():
    ...
    try:
        ...
        # This should work
        await backend2.query("*IDN?")
        
        # This should fail - wrong command
        try:
            await backend2.query("WRONG:CMD?")
...
async def test_session_files():
    ...
    try:
        ...
        await psu_backend.connect()
        psu_id = await psu_backend.query("*IDN?")
        await psu_backend.write("VOLT 3.3")
        await psu_backend.close()
        print(f"✓ PSU replay successful: {psu_id}")
        
        # Test DMM backend
        dmm_backend = ReplayBackend(loaded_data["dmm"]["log"], "dmm")
        await dmm_backend.connect()
        dmm_id = await dmm_backend.query("*IDN?")
        reading = await dmm_backend.query("READ?")
        await dmm_backend.close()
...
if __name__ == "__main__":
    async def main():
        success1 = await test_replay_backend()
        success2 = await test_session_files()
