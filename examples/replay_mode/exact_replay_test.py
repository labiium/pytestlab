...
async def main(bench):
    ...
    await osc._backend.write(":TIMebase:SCALe 0.001")
    ...
    for i, voltage in enumerate(voltages):
        ...
        # Set voltage
        await psu.set_voltage(1, voltage)  # VOLT X, (@1)
        
        # Wait for settling
        await asyncio.sleep(0.5)
        
        # Read PSU measurements (exact sequence from recording)
        measured_voltage = await psu.read_voltage(1)  # MEAS:VOLT? (@1)
        measured_current = await psu.read_current(1)  # MEAS:CURR? (@1)
    ...
    await psu.output(1, False)     # OUTP:STAT OFF, (@1)
    await psu.set_voltage(1, 0.0)  # VOLT 0.0, (@1)
