...
async def test_real_instruments_recording():
    ...
    try:
        ...
        await perform_measurement_sequence(psu, osc)
...
async def test_replay_session(session_file: Path):
    ...
    try:
        ...
        psu_replay = await AutoInstrument.from_config(
            "keysight/EDU36311A",
            backend_override=psu_backend
        )
        await psu_replay.connect_backend()
        
        osc_replay = await AutoInstrument.from_config(
            "keysight/DSOX1204G",
            backend_override=osc_backend
        )
        await osc_replay.connect_backend()
        
        print("Replaying measurement sequence...")
        
        # Replay the same measurement sequence
        await perform_measurement_sequence(psu_replay, osc_replay)
        
        print("✓ Replay completed successfully")
        
        await psu_replay.close()
        await osc_replay.close()
...
async def perform_measurement_sequence(psu, osc):
    ...
    await osc.set_time_axis(0.001, 0.0)  # 1ms/div, centered
    await osc.set_channel_axis(1, 1.0, 0.0)  # 1V/div, no offset
    await osc.display_channel(1, True)  # Enable channel 1
    
    # Set trigger
    await osc.configure_trigger(1, 2.5, source="CHAN1", mode="EDGE")
    ...
    try:
        ...
        for i, voltage in enumerate(voltages):
            ...
            actual_voltage = await psu.read_voltage(1)
            actual_current = await psu.read_current(1)
...
async def test_mismatch_detection(session_file: Path):
    ...
    try:
        ...
        try:
            await psu_replay.set_voltage(1, 999.0)  # Wrong voltage value
...
async def main():
    ...
    session_file = await test_real_instruments_recording()
    if not session_file:
        print("✗ Recording failed, cannot continue with replay tests")
        return 1
    
    # Step 2: Test replay
    replay_success = await test_replay_session(session_file)
    if not replay_success:
        print("✗ Replay test failed")
        return 1
    
    # Step 3: Test mismatch detection
    mismatch_success = await test_mismatch_detection(session_file)
