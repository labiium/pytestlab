...
async def run_transistor_iv_sweep():
    ...
    async with MeasurementSession(
        ...
        psu1 = await session.instrument("psu_base", "keysight/EDU36311A", simulate=True)
        psu2 = await session.instrument("psu_collector", "keysight/EDU36311A", simulate=True)
        dmm = await session.instrument("dmm", "keysight/EDU34450A", simulate=True)
        
        # Define a measurement function using the @session.acquire decorator
        @session.acquire
        async def measure_collector_current(I_base: float, V_ce: float, temperature: float, 
            ...
            await psu_base.set_voltage(1, V_base)
            await psu_base.set_current(1, 0.01)  # 10mA current limit
            
            # 2. Set up collector voltage with psu_collector
            await psu_collector.set_voltage(1, V_ce)
            await psu_collector.set_current(1, 0.5)  # 500mA current limit
            
            # 3. Turn on power supplies
            await psu_base.output(1, True)
            await psu_collector.output(1, True)
            
            # 4. Wait for circuit to stabilize
            await asyncio.sleep(0.01)  # 10ms in real test would be longer
            ...
            measurement = await dmm.measure_current_dc()
            ...
            await psu_base.output(1, False)
            await psu_collector.output(1, False)
...
async def main():
    ...
    experiment = await run_transistor_iv_sweep()
