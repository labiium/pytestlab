...
class _Extractor(ast.NodeVisitor):
    ...
    def visit_ClassDef(self, node: ast.ClassDef) -> None:  # noqa: N802
        ...
        if any(
            b and ("Instrument" in b or "SCPIMixin" in b or "SCPIUnknownMixin" in b) for b in bases
        ):
    ...
    def visit_Assign(self, node: ast.Assign) -> None:  # noqa: N802
        ...
        if isinstance(value, ast.Call):
            ...
            if call_name and (
                "control" in call_name or "measurement" in call_name or "setting" in call_name
            ):
    ...
    # ---- property parser ---------------------------------------------------
    def _parse_property_call(
        self, name: str, call_name: str, node: ast.Call
...
def _build_yaml(extracted: ExtractedInstrument) -> dict[str, Any]:
    ...
    for prop in extracted.properties:
        if prop.kind in ("control", "setting") and prop.set_command:
            ...
            if prop.values and isinstance(prop.values, dict) and prop.map_values:
                yaml_cmd["enums"] = {
                    "value"
                    if len(prop.values) == 2 and all(isinstance(v, bool) for v in prop.values)
                    else "state": {str(k).lower(): v for k, v in prop.values.items()}
                }
    ...
    if extracted.channels:
        profile["channels"] = [
            {"channel_id": i + 1, "description": ch.name} for i, ch in enumerate(extracted.channels)
        ]
...
def _build_py_method(prop: ExtractedProperty) -> str | None:
    ...
    if prop.kind in ("control", "measurement") and prop.get_command:
        ...
        if prop.channel_aware:
            args = ["channel: int = 1"]
        lines.append(
            f"    def {method}(self{', ' + ', '.join(args) if args else ''}) -> {arg_type}:"
        )
