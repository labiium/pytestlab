...
class TestSessionRecordingBackend:
    ...
    @pytest.mark.asyncio
    async def test_initialization(self, mock_backend, temp_output_file):
    ...
    @pytest.mark.asyncio
    async def test_query_recording(self, recording_backend, temp_output_file):
        """Test query command recording."""
        # Execute a query
        result = await recording_backend.query('*IDN?')
    ...
    @pytest.mark.asyncio
    async def test_write_recording(self, recording_backend):
        """Test write command recording."""
        # Execute a write
        await recording_backend.write('CURR 0.1, (@1)')
    ...
    @pytest.mark.asyncio
    async def test_mixed_command_sequence(self, recording_backend):
        """Test recording a sequence of mixed commands."""
        # Execute sequence
        idn_result = await recording_backend.query('*IDN?')
        await recording_backend.write('CURR 0.1, (@1)')
        error_result = await recording_backend.query(':SYSTem:ERRor?')
        volt_result = await recording_backend.query('MEAS:VOLT? (@1)')
        await recording_backend.write('OUTP:STAT ON, (@1)')
    ...
    @pytest.mark.asyncio
    async def test_session_file_creation(self, recording_backend, temp_output_file):
        """Test that session file is created with correct format."""
        # Record some commands
        await recording_backend.query('*IDN?')
        await recording_backend.write('CURR 0.1, (@1)')
        await recording_backend.query(':SYSTem:ERRor?')
        
        # Save session
        await recording_backend.save_session('keysight/EDU36311A')
    ...
    @pytest.mark.asyncio
    async def test_timestamp_ordering(self, recording_backend):
    ...
    @pytest.mark.asyncio
    async def test_backend_error_propagation(self, temp_output_file):
        ...
        with pytest.raises(RuntimeError, match="Backend error"):
            await recording_backend.query('ERROR_CMD')
        
        # Test write error propagation
        with pytest.raises(ValueError, match="Write error"):
            await recording_backend.write('ERROR_CMD')
        
        # Test successful commands still work
        result = await recording_backend.query('GOOD_CMD')
        assert result == "OK"
    
    @pytest.mark.asyncio
    async def test_multiple_profile_keys(self, mock_backend, temp_output_file):
        ...
        await psu_backend.query('*IDN?')
        await psu_backend.write('CURR 0.1, (@1)')
        await psu_backend.save_session('keysight/EDU36311A')
        
        # Record commands for oscilloscope
        await osc_backend.query('*IDN?')
        await osc_backend.write(':CHANnel1:SCALe 1.0')
        await osc_backend.save_session('keysight/DSOX1204G')
...
class TestSessionRecordingBackendEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_empty_command_sequence(self, recording_backend, temp_output_file):
        """Test saving session with no recorded commands."""
        # Save without recording any commands
        await recording_backend.save_session('test/profile')
    ...
    @pytest.mark.asyncio
    async def test_invalid_output_file_path(self, mock_backend):
        ...
        with pytest.raises(FileNotFoundError):
            await backend.save_session('test/profile')
    
    @pytest.mark.asyncio
    async def test_concurrent_recording(self, recording_backend):
        """Test thread safety of concurrent command recording."""
        async def record_commands(start_index):
            for i in range(5):
                await recording_backend.query(f'CMD_{start_index}_{i}?')
                await recording_backend.write(f'SET_{start_index}_{i} VALUE')
        ...
        assert len(recording_backend._command_log) == 30  # 3 tasks × 5 commands × 2 (query+write)
...
@pytest.mark.asyncio
async def test_session_recording_backend_integration():
    ...
    try:
        ...
        idn = await recording_backend.query('*IDN?')
        ...
        for voltage in [1.0, 2.0, 3.0]:
            await recording_backend.write(f'VOLT {voltage}, (@1)')
            measured_v = await recording_backend.query('MEAS:VOLT? (@1)')
            measured_i = await recording_backend.query('MEAS:CURR? (@1)')
            
            # Verify measurements match set values
            assert f'{voltage:.8E}' in measured_v
        
        await recording_backend.write('OUTP:STAT OFF, (@1)')
        await recording_backend.write('VOLT 0.0, (@1)')
        
        # Save the session
        await recording_backend.save_session('test/realistic_psu')
