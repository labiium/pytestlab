...
class TestAutoInstrumentBackendOverride:
    ...
    @pytest.mark.asyncio
    async def test_instrument_methods_with_backend_override(self, temp_profile_file):
        ...
        # Test query method
        result = await instrument._backend.query('*IDN?')
        assert result == "MOCK_RESPONSE_*IDN?"
        assert '*IDN?' in mock_backend.queries
        
        # Test write method
        await instrument._backend.write('CURR 0.1')
        assert 'CURR 0.1' in mock_backend.writes
    
    @pytest.mark.asyncio
    async def test_replay_backend_integration(self, temp_profile_file, temp_session_file):
...
class TestBackendOverrideEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_backend_override_state_isolation(self, temp_profile_file):
        ...
        await instrument1._backend.query('CMD1')
        await instrument2._backend.query('CMD2')
        await instrument1._backend.write('WRITE1')
        await instrument2._backend.write('WRITE2')
...
@pytest.mark.asyncio 
async def test_backend_override_with_session_recording():
    ...
    try:
        ...
        try:
            ...
            # Use the instrument
            result = await instrument._backend.query('*IDN?')
            await instrument._backend.write('TEST:CMD 1')
