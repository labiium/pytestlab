...
class DummyBackend:
    ...
    def write(self, cmd: str): pass
    def query(self, cmd: str, delay: float | None = None) -> str: return "OK"
    def query_raw(self, cmd: str, delay: float | None = None) -> bytes: return b"#10OK"
    def close(self): pass
    def set_timeout(self, timeout_ms: int): pass
    def get_timeout(self) -> int: return 1000
...
def test_feature_mapping_validation_passes_when_present():
    ...
    # Should not raise
    inst._validate_features_against_scpi({"myfeat": {"required_scpi": ["alpha"], "optional_scpi": ["beta"]}}, strict=True)
...
def test_feature_mapping_validation_raises_when_missing():
    ...
    with pytest.raises(RuntimeError):
        inst._validate_features_against_scpi({"myfeat": {"required_scpi": ["missing_cmd"]}}, strict=True)
