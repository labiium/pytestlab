...
def test_json_schema_generation_includes_commands_and_queries():
    scpi = SCPISection(
        commands={"foo": CommandSpec(template=":FOO {x}")},
        queries={"bar": CommandSpec(template=":BAR?", response=ResponseSpec(type="str"))},
        feature_mappings={"feature1": {"required_scpi": ["foo"], "optional_scpi": ["bar"]}},
    )
