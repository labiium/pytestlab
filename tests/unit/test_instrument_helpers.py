...
@pytest.mark.parametrize(
    "header, data, expected_array, dtype, is_big_endian",
    [
        ...
        (
            b"#18",
            b"\x3f\x80\x00\x00\x40\x00\x00\x00",
            np.array([1.0, 2.0], dtype=np.float32),
            np.float32,
            True,
        ),
        ...
        (
            b"#210",
            b"abcdefghij",
            np.array([97, 98, 99, 100, 101, 102, 103, 104, 105, 106], dtype=np.uint8),
            np.uint8,
            False,
        ),
...
@pytest.mark.parametrize(
    "malformed_bytes, dtype, is_big_endian, error_type",
    [
        # Malformed headers
